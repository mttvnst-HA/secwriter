...
## Untrusted .SEC Input → block.html Is HTML-Encoded

A .SEC file is untrusted input (any collab client can upload one; the server re-parses it and every peer renders the result). The invariant: **`block.html` (and table cell `text`, ref `org`/`rid`/`rtl`) is always HTML-encoded text — a literal `<`, `>`, `&` in the document is carried as `&lt;`/`&gt;`/`&amp;`.** All three HTML producers agree on this: `sec-parser.js`'s builders (`elemToHtml`, `elemToHtmlNoTab`, `elemToTblHtml`) route every text node through `escapeHtmlText`, and `pmdoc-html.js`/`ytext-html.js` `escapeHtml` on serialization. The XML parser has already decoded entities by the time a builder sees `textContent`, so a .SEC containing `&lt;img src=x onerror=…&gt;` yields a text node that IS `<img …>` — without the escape it lands raw in block.html and reaches `innerHTML` / `dangerouslySetInnerHTML` (TableBlock, TitleBlock, PreformattedBlock, MarkSuggestions, comments.js) and the print windows' `document.write`. Downstream consumers parse block.html as HTML, and `sec-serializer.js`'s `htmlToSgml` decodes via DOMParser then re-escapes for XML (`escapeXmlText`) — exactly one level of escaping in the file, never `&amp;lt;`. The same escape also fixes a fidelity bug: real UFGS content like `<Insert shape>` (08 44 00) and `<25` in MET/ENG cells (23 07 00, 22 00 00) was previously parsed as markup and dropped on round-trip. Pinned by the "Text-node HTML escaping" tests in `src/lib/__tests__/sec-parser.test.js`, the normal-path decode test in `sec-serializer.test.js`, and the escaped-angle-bracket round-trip in `sec-roundtrip.test.js`. **Adding a fourth builder or a new text-node push in the parser must go through `escapeHtmlText`.** The TAI `data-opt` attribute goes through the sibling `escapeHtmlAttr`. CodeQL's js/xss (alert #4) and js/xml-bomb (alert #22, mitigated by the DOCTYPE/ENTITY guard at the top of `parseSEC`) queries have no sanitizer model for hand-written escapers/guards, so they stay flagged at the `parseFromString` call; both are dismissed as false positives pointing here.
