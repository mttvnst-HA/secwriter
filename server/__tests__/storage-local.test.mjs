...
describe('LocalStorageBackend', () => {
  ...
  it('readRoom returns partial data when .SEC or .comments.json missing', async () => {
    ...
    // Write only the .ydoc file directly (simulating a legacy room)
    fs.writeFileSync(path.join(dir, 'legacy.ydoc'), Buffer.from([0xFF]));

    const result = await backend.readRoom('legacy');
  ...
  it('listRooms excludes .tmp, .corrupt, .oversize variants', async () => {
    const dir = freshDir();
    const backend = new LocalStorageBackend(dir);
    await backend.writeRoom('real', { ydocBytes: Buffer.from([1]), secBytes: null, commentsJson: null });
    // Write fake variant files that should be excluded
    fs.writeFileSync(path.join(dir, 'real.ydoc.tmp'), Buffer.from([0]));
    fs.writeFileSync(path.join(dir, 'ghost.ydoc.corrupt.12345'), Buffer.from([0]));
    fs.writeFileSync(path.join(dir, 'ghost2.ydoc.oversize.12345'), Buffer.from([0]));
  ...
  it('sanitizes room names', async () => {
    const dir = freshDir();
    const backend = new LocalStorageBackend(dir);
    await backend.writeRoom('../../etc/passwd', { ydocBytes: Buffer.from([1]), secBytes: null, commentsJson: null });
  ...
  it('writeRoom is atomic — no partial artifacts on write failure', async () => {
    ...
    await backend.writeRoom('atomic-test', {
      ydocBytes: Buffer.from([1]),
    ...
    const ydocPath = path.join(dir, 'atomic-test.ydoc');
    ...
    // Read .SEC directly — it should still be the original or absent
    const secPath = path.join(dir, 'atomic-test.SEC');
