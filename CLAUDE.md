...
10. **Before claiming "no E2E regressions," run the FULL `editor.spec.js` and `collab.spec.js` under `--project=chromium` (the PM EditorView project).** Post-1i-a, `chromium` is the sole CI merge gate; `chromium-legacy` remains in `playwright.config.js` as a transitional local safety net through 1i-b.1 and is deleted in 1i-b.2. Spot-checking specific tests rather than the full suite is how the legacy-mode `snapshotText` regression in PR #95 reached CI — run the whole spec file. **During the 1i-b.1 window, any change that touches `EditableBlock.jsx`, `useUndoableBlocks`, `FloatingToolbar` legacy branches, or other legacy-path code must also pass `--project=chromium-legacy` locally before merge** — CI no longer gates it, so the regression class PR #95 introduced can only be caught by the local spot-check. **Baseline under `chromium` is the 9 parallel-load flakes catalogued in [docs/superpowers/notes/1i-a-pm-failures.md](docs/superpowers/notes/1i-a-pm-failures.md)** (zero persistent failures; all 9 pass in isolated rerun). The older 1h-era "~29 parallel-load flakes" figure described the chromium-legacy+chromium combined run and no longer applies. To distinguish regression from flake: `git stash`, re-run the failing tests by `--grep` under `--project=chromium`; if they fail at baseline too, it's a flake. Trust isolated runs over the full-suite diff.
...
6. **Undo/redo coupling.** Post-1i-b.1, `tcState` no longer rides on `useUndoableBlocks`' snapshot stack — it lives in App-level `useState` and the hook snapshots `{ blocks }` only. Accepted regression: Ctrl+Z across a TC enable/disable boundary no longer rolls back the toggle (treated as explicit UI, not a typing frame). The Yjs UndoManager (in-room via `collab.js`, out-of-room via `useLocalSubstrateUndoManager`) is the source of truth for keystroke-grain and structural undo; `useUndoableBlocks` remains as a transitional fallback through the 1i-b.2 deletion sweep.
