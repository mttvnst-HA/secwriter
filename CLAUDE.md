...
2. **`onRevisionAction` / `onRefreshTcSnapshot`** are the prop-layer dispatchers used by FloatingToolbar (both legacy and PM paths), EditableBlock's del popup (legacy), and PmEditableBlock's `handleClick` del popup (1f.8). The App-side `handleRevisionAction` writes substrate + setBlocks + tc.applyResolveAtBlock; the App-side `handleRefreshTcSnapshot` (1f.9, FloatingToolbar PM-mode inline TC only) skips the substrate write because PM dispatch already wrote it. See item 6 for the full seam map.
...
6. **Del elements** have `contentEditable="false"` to prevent caret entry, and `cursor: pointer` for click-to-show popup. Diff annotation (turning the snapshot diff into `<ins>`/`<del>` DOM marks) stays in EditableBlock — the module remains pure and DOM-free. **PM path (1f.8):** del-click popup lives in `PmEditableBlock`'s `handleClick` alongside the comment-click handler; accept/reject route through `applyDelAction` (pure HTML mutator at `src/lib/pm-del-popup.js`) and `onRevisionAction` (origin `'local-publish'` for UndoManager coverage). The mutator identifies the target del by DOM index, not by PM mark equality, because adjacent del marks with different `authorId` are separate marks in PM's schema. **FloatingToolbar PM path (1f.9):** the five mark-application verbs (format, inline-mark, revision-apply, inline-revision-resolve, change-case) dispatch PM transactions via `pm-toolbar.js`'s pure verb functions when `block-registry.getBlockView(blockId)` returns a non-null `EditorView`. For four of the five verbs, `flushPendingUpdateById(blockId)` is called after dispatch to close the 400ms debounce window so App's `blocks` array reflects substrate synchronously. **Inline accept/reject is the exception:** it uses `cancelPendingUpdateById(blockId)` (debounce clear without firing onUpdate) plus a new `onRefreshTcSnapshot` App handler that calls `resumeHistory() + setBlocks + setTcState` (no `setBlockHtml` — PM dispatch already wrote the substrate via ySyncPlugin). `cancelPendingUpdate` is required because handleBlockUpdate runs outside any `resumeHistory()` window, so a debounce-driven late setBlocks would land while `useUndoableBlocks` is paused and either capture no snapshot or capture a snapshot of the post-action state — both shadow the intended undoable frame. Out-of-room mode therefore gets a proper undoable frame for inline TC accept/reject via `useUndoableBlocks`. In-room mode does not — `collab.tryUndo` prefers the Yjs UndoManager which only tracks `'local-publish'` origin ops, and `setBlockHtml` after ySyncPlugin's write is a no-op delta that produces no Yjs ops. This gap is part of the broader PM-mode in-room undo limitation tracked alongside the existing Ctrl+Y redo off-by-one. This whole PM-toolbar seam is distinct from the 1f.8 del-popup's `onRevisionAction` path — the del-popup mutates serialized HTML and DOES require `setBlockHtml`.
...
10. **Known limitation — y-prosemirror drops the `comment` mark in `prosemirrorToYXmlFragment` (issue #64).** Verified by drilling YXmlText deltas: `inlineMark`/`bold` survive the PM-doc → Y.XmlFragment write, `comment` is silently folded into surrounding text. `handleCommentCreate` (App.jsx:672) currently calls only `setBlocks` and skips `setBlockHtml`, which is why this hasn't surfaced in practice — comment marks live in React state, PM's view reads from substrate, and the two are allowed to disagree for the comment layer specifically. Until #64 is fixed, **do not route comment-creating flows through `setBlockHtml`**. **FloatingToolbar PM path (1f.9):** the comment button is the only toolbar verb that did NOT convert to PM transactions in 1f.9 — see the `// XXX(#64)` comment in `FloatingToolbar.jsx`. In PM mode, the `range.surroundContents(<span class="mark-comment">)` mutation is reverted by PM's DOMObserver on next render, but the metadata still reaches `commentsState` via `onCommentCreate`. The disagreement (no visible highlight, present metadata) is the same shape this note describes.
