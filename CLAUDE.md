...
3. **`resumeHistory()` before any click-driven `setBlocks`.** `useUndoableBlocks` auto-pauses for ~400ms after each input flush. A click handler (del-popup accept/reject, toolbar revision verb, accept-all, etc.) that fires `setBlocks` while paused captures NO snapshot — undo silently breaks. Never wire a click-driven action directly to `handleBlockUpdate` (which has no `resumeHistory()`); use the appropriate sibling from item 2.
