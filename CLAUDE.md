...
10. **Before claiming "no E2E regressions," run the FULL `editor.spec.js` and `collab.spec.js` under `--project=chromium`.** Single-project gate post-1i-b.2; PmEditableBlock is the only editor. Spot-checking specific tests rather than the full suite is how the legacy-mode `snapshotText` regression in PR #95 reached CI — run the whole spec file. **Baseline under `chromium`** is the parallel-load flake set tracked at [#126](https://github.com/mttvnst-HA/secwriter/issues/126). Issue [#114](https://github.com/mttvnst-HA/secwriter/issues/114) (`:404` / `:918` / `:1183`, now at lines `:462` / `:976` / `:1247`) closed COMPLETED 2026-05-19 after PR #115's helper fix; 2026-05-20 verification confirmed 9/9 isolated passes at W=1 repeat=3 — full-suite parallel-load status open at [#145](https://github.com/mttvnst-HA/secwriter/issues/145). Recent #126 closures: PR #142 closed revision-stats addition-count; PR #143 closed `:1209` TC-commit race. To distinguish regression from flake: `git stash`, re-run the failing tests by `--grep` under `--project=chromium`; if they fail at baseline too, it's a flake. Trust isolated runs over the full-suite diff. The "zero persistent failures" claim in `docs/superpowers/notes/1i-a-pm-failures.md` is its 2026-05-16 snapshot — see that file's header for current scope.
...
- `slash-menu.js` — PM `Plugin` with `{open, filter, fromPos}` state; popup is the React `SlashMenu.jsx`, portal-mounted at `document.body` with `position: fixed`, anchored via `view.coordsAtPos(fromPos)`. The PM editor's contentEditable carries combobox ARIA (`role=combobox`, `aria-activedescendant`); the menu carries `role=listbox` (combobox-with-listbox-popup pattern).
...
See [ADR-0014](docs/adr/0014-collab-server-yjs-relay.md) (and [ADR-0001](docs/adr/0001-server-uses-commonjs.md) for CJS, [ADR-0002](docs/adr/0002-pin-y-websocket-v1.md) for the y-websocket pin). Real-time multi-user editing via Yjs + y-websocket. Server lives in `server/`:

- `server/collab-server.cjs` — y-websocket relay. `createCollabServer({ storage })` factory; CLI entry gated by `if (require.main === module)` so tests can `require()` without binding a port.
