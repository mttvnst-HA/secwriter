...
7. **`compliance-highlight.js`** — DOM helpers for the active-group `.compliance-highlight` span injection. Pure compute (`findHighlightTargetsInBlock` walks text nodes, applies word-boundary check, skips `<del class="mark-del">` and existing highlights) + side-effect mutate (`injectHighlightSpans` / `clearHighlightSpans`). The highlight effect lives at App level (`useEffect([complianceOpen, complianceState.activeGroup, complianceState.result])`) — single seam matching linting's `CSS.highlights` pattern. Cleanup unwraps spans on activeGroup change, panel close, or unmount.
