...
7. **CI-only flakes are timing races.** When a test fails only on a CI runner but passes 10×/10 locally, do NOT keep re-running locally — write a deterministic regression test that forces the race (e.g., manually mutate shared state mid-`await`). See `server/__tests__/collab-server.test.mjs` for the pattern (historically used force-deletion of the y-websocket docs Map during a slow-storage read to expose the eviction race; the Hocuspocus equivalents use a two-provider loopback and a slow-storage read that forces `onLoadDocument` to race against a concurrent connect).
