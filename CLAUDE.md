...
10. **Before claiming "no E2E regressions," run the FULL `editor.spec.js` and `collab.spec.js` under `--project=chromium` (the PM EditorView project).** Post-1i-a, `chromium` is the sole CI merge gate; `chromium-legacy` remains in `playwright.config.js` as a transitional local safety net through 1i-b.1 and is deleted in 1i-b.2. Spot-checking specific tests rather than the full suite is how the legacy-mode `snapshotText` regression in PR #95 reached CI — run the whole spec file. **Baseline at the 1h Q36 head has ~29 parallel-load flakes** (≈18 collab tests fail, 4 PM editor tests fail persistently, ~5 additional editor tests fail only under load); after the 1i-a PM audit the baseline list under `chromium` is the 9 flake tests catalogued in the audit doc. To distinguish regression from flake: `git stash`, re-run the failing tests by `--grep` under `--project=chromium`; if they fail at baseline too, it's a flake. Trust isolated runs over the full-suite diff.
