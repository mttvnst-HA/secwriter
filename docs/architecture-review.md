...
4. **`useReviewPanels` (lint+compliance)** — the deferred pair from #3's re-grill. Shared ignore store + CSS.highlights registry + suspend coupling. Wider than #3: `lintingState` is bidirectionally co-owned with `useCollabSession` (reactive publish + 5 inbound `setLintingState` callbacks) and leaks raw `setLintingState` into 3 render sites, so it needs intent-method design, not a mechanical lift. Absorbs `lastComplianceScrollRef`.
