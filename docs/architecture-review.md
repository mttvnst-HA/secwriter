...
## 8. Blocks reducer + dispatcher (2026-05-19)

**Status:** Landed — `src/lib/blocks.js` is now a pure reducer over `Block[]` exposing 21 verbs and a single `dispatchBlocksVerb` dispatcher. Each verb returns `{ state, effects: { framing, substrateWrites, flush, focus } }` where `framing` is a tagged union (`{ kind: 'newFrame' }` for forceFrame-then-write, `{ kind: 'wrappedFrame', writes }` for withUndoFrame-wraps-N-writes-as-one-undo-frame, or `null`). The descriptor models html-slot writes only; structural changes (create/delete/reorder) ride the implicit `setBlocks → applyBlocksToYDoc` diff path. The dispatcher mutates `blocksRef.current` synchronously after `setBlocks(state)` so sequential dispatches (Replace All, Remove All Orphaned, multi-step compliance accept) see the latest state mid-loop instead of the pre-loop snapshot — without this, the second-and-later calls in a synchronous tick clobber each other. App.jsx adds one thin `dispatchBlocks(compute, opts)` closure that wires `yStore` and `framing` (collab vs localUndo) at call time, then 14 App handlers + 7 inline JSX handlers all collapse into one-line dispatcher invocations. The handler trio `handleBlockUpdate` / `handleBlockUpdatePmSync` / `handleBlockUpdateWithSync` survives as three distinct verbs (`updateBlockHtml`, `updateBlockHtmlPmSync`, `updateBlockHtml` again — `WithSync` is now the same verb since the pre-1i-b.2 `getBlockHandle(id).setHtml(html)` registry mirror was dead code in a PM-only world).
