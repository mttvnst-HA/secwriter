...
**Direction.** Lift each intent behind its own hook (mirror `useCollabSession`); App keeps render + prop wiring. Candidates #5 (landed) and #2 (landed [#284](https://github.com/mttvnst-HA/secwriter/pull/284)) were the first two slices. Remaining slices, cleanest-first:
1. **`useFileSession`, output half** — landed [#286](https://github.com/mttvnst-HA/secwriter/pull/286). 7 save/export/download handlers, pure readers, ~250-LOC lift.
2. **`useFileSession`, input half** — landed [#287](https://github.com/mttvnst-HA/secwriter/pull/287). **Re-grilled: the naive "move `loadSECContent` into the hook" is a shallow seam** — `loadSECContent` is a whole-document reset (7 App-state setters); threading them through a hook makes it own nothing. Deep module found instead: the file-INPUT *I/O shell* (drag-drop parsing, two FileReaders, windows-1252 decode, lint-companion staging) hidden behind one `onFileLoaded(text,name,lintText)` callback. `loadSECContent`/`extractMetadata`/`applyLintSidecarPayload` stay in App. `clearHistoryRef` **not** dissolved here — it's a declaration-order artifact (loadSECContent before useCollabSession), orthogonal to file I/O; separate reorder cleanup if ever worth it.
3. **`useComments`** — landed [#288](https://github.com/mttvnst-HA/secwriter/pull/288). **Re-grilled (independent review): the backlog's `useReviewPanels` bundled comments+compliance+lint, but comments share ZERO state with lint/compliance** (grep-verified) — they couple to the substrate/collab/PM axis and use PM mark-spans, not the CSS.highlights registry; the only link is the `showComments` XOR `complianceOpen` panel toggle. So comments extract alone. The hook owns comment **state + the 7 handlers**; the **4 effects stay in App at their original declaration positions** — they are effect-order-sensitive (the hook is called early, before `useFileSession`, to supply `comments`; relocating the effects ahead of App's others is a Rule #12 hazard). `showComments` (layout toggle) + `effectiveIdentity` (shared) stay in App. lint+compliance — the pair that genuinely share the CSS.highlights registry + ignore sub-store + suspend coupling — are a **separate later slice** (`useReviewPanels`, lint+compliance only; can sub-slice a narrow CSS.highlights-registry effect-hook as its spine).
4. **`useCompliancePanel`** — landed [slice 4a]. **Re-grilled: `useReviewPanels` (lint+compliance as one) is the shallow-grouping trap again** — grouping by the "review panels" UI label, not the coupling axis. The two split cleanly: `complianceState` is genuinely private (`comp.createInitial()` scan result — NOT collab-published, NOT file-exported, read only by the toolbar toggle + CompliancePanel render + the highlight/scroll effect), whereas `lintingState` is collab-published DOCUMENT state (read OUT by `useCollabSession`'s 3 publish slices + `useFileSession` sidecar export + `useBlockLinting`'s per-keystroke raw `dispatch`; written IN by 4 peer remote-merge callbacks). So compliance extracts here as the deep, self-contained half (owns `complianceOpen`/`complianceState`/`lastComplianceScrollRef` + the `compliance-active` highlight effect); the only lint wire — `complianceOpen → linting.setSuspended` — stays in App as a 1-line lint write. Reorder of the highlight effect is Rule-#12-safe: `compliance-active` is a disjoint CSS.highlights key from lint's tiers.
4b. **`useLinting`** — the deferred lint half. A **custodian** hook mirroring `useComments` (returns `lintingState` + raw `dispatch` + the 4 remote-merge callbacks for App to thread back into `useCollabSession`/`useFileSession`/`useBlockLinting`), NOT a behavior-encapsulating one — the child hooks compose reducer verbs (`setBlockFindings` with computed args), so named intent methods can't replace the raw dispatch. Absorbs the prune + tier-highlight + suspend effects. The publish-gate refs (`lastPublishedLint*Ref`) already live in `useCollabSession` — don't move them.
5. **editor-actions** — deepest coupling; re-grill after the above shrink it (may not earn full extraction).
