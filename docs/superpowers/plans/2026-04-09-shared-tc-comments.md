...
**Architecture:** Add two new top-level Yjs shared types to the per-room `Y.Doc`: `yTc` (enabled flag + snapshots Y.Map) and `yComments` (Y.Map<id, Y.Map> with `entries: Y.Array<Y.Map>`). Extend `CollabSession` with publish/read helpers for both, observed via the existing `afterTransaction` handler using two new change detectors that mirror the existing `blocksChanged` / `metaChanged` routing. React `trackChanges` / `tcSnapshots` / `comments` state becomes a derived view of Y.Doc when `inRoom`. Author attribution on `<ins>`/`<del>` marks is applied by extending `annotateDomWithDiff(container, snapshotText, author?)` in `src/lib/text-diff.js` — this is SIM's actual DOM-producing TC rendering path. When an `author` is passed in, every created `<ins>` wrapper and `<del>` node gets `data-author-id`, `data-author-name`, `data-author-color`, and a `style="--author-color:<color>"` inline declaration. Neither `yTc` nor `yComments` is tracked by `Y.UndoManager` (matches single-user semantics).

**Historical note (Task 4 iteration):** An earlier version of this plan assumed `refineWordDiff(oldText, newText)` was SIM's HTML-producing TC rendering path, and the first Task 4 commit added a string overload to match. That assumption was wrong — `refineWordDiff(ops)` takes a diff-ops array and SIM's real rendering happens inside `annotateDomWithDiff`, which creates DOM nodes programmatically. The string overload was pure dead code and has been reverted. Task 4 was redone to extend `annotateDomWithDiff` and its internal `wrapRangeInElement` helper with an optional `author` parameter, which is the single correct place for author attribution.
...
| `src/lib/text-diff.js` | Optional `author` parameter on `annotateDomWithDiff` (the real DOM-producing path, NOT `refineWordDiff`). New private `applyAuthorToElement` helper applies `data-author-id/name/color` + `--author-color` inline style to every created `<ins>` wrapper and `<del>` node. `wrapRangeInElement` gains an `author` parameter threaded from `annotateDomWithDiff`. |
| `src/lib/__tests__/text-diff.test.js` | 3 new tests for author attribute emission via `annotateDomWithDiff` (uses linkedom DOM polyfill) |
