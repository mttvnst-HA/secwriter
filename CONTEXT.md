...
**schemaVersion** — Integer in `yMeta` marking the substrate version. v1 = html slot is Y.Text. v2 = html slot is Y.XmlFragment. Clients enforce `MAX_SUPPORTED_SCHEMA_VERSION` (defined in `src/lib/session-coordination.js`; read the constant there rather than assuming a value); a remote `schemaVersion > MAX_SUPPORTED_SCHEMA_VERSION` surfaces an `'incompatible'` connection status and gates editing.
